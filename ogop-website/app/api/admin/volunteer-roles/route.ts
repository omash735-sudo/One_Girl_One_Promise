...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await sql`
      INSERT INTO volunteer_roles (title, description, icon, display_order)
    ...
    console.error('POST volunteer role error:', error)
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error('PUT volunteer role error:', error)
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await sql`DELETE FROM volunteer_roles WHERE id = ${id}`
    
    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('DELETE volunteer role error:', error)
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
}
